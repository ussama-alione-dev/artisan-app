...
export default function ArtisansPage() {
  ...
  useEffect(() => { fetch(); }, [specialty, city, availableOnly]);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          {loading ? '…' : `${artisans.length} professional${artisans.length !== 1 ? 's' : ''} found`}
        </p>
      </div>
      ...
      <div className="card p-4 mb-8 flex flex-wrap items-center gap-3">
        ...
        <select
          className="input" style={{width:'auto',minWidth:'160px'}}
        ...
        <div className="relative">
          ...
          <input
            className="input pl-9" style={{minWidth:'180px'}}
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          >
            <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${availableOnly ? 'translate-x-4' : ''}`} />
          </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-24 text-slate-400">
          <Loader2 size={28} className="animate-spin mr-3" /> Loading artisans…
        </div>
      ) : artisans.length === 0 ? (
        <div className="text-center py-24">
          <p className="text-4xl mb-3">🔍</p>
          <p className="font-semibold text-slate-700 text-lg">No artisans found</p>
