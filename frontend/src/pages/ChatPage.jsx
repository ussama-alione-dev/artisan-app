...
export default function ChatPage() {
  ...
  function sendMessage(e) {
    e.preventDefault();
    if (!text.trim()) return;
    socket.emit('send_message', { roomId, senderId: user.id, senderName: user.name, text: text.trim() });
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-slate-200">
        ...
        <div className="flex-1">
          <p className="font-semibold text-slate-900 text-sm">Private Chat</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-1 bg-slate-50">
        ...
        {grouped.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${mine ? 'justify-end' : 'justify-start'} ${msg.isFirst ? 'mt-4' : 'mt-0.5'}`}>
              <div className={`flex flex-col ${mine ? 'items-end' : 'items-start'} max-w-[72%]`}>
                {!mine && msg.isFirst && (
                  <p className="text-xs font-semibold text-slate-500 mb-1 ml-1">{msg.senderName}</p>
                )}
                ...
                {msg.isLast && (
                  <p className={`text-[11px] mt-1 mx-1 ${mine ? 'text-slate-400' : 'text-slate-400'}`}>
