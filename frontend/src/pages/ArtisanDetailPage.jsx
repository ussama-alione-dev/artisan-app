...
import { MapPin, Phone, Euro, MessageCircle, Send, ChevronLeft, Loader2, Star } from 'lucide-react';
import { api } from '../api';
...
export default function ArtisanDetailPage() {
  ...
  if (!artisan) return (
    <div className="text-center py-20 text-slate-500">Artisan not found.</div>
  );

  const meta = SPECIALTY_META[artisan.specialty] || { icon: '🛠️', bg: 'bg-slate-50', text: 'text-slate-700' };
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 animate-fade-in">
      {/* Back */}
      <Link to="/artisans" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 mb-6 transition-colors">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="px-6 pb-6">
          {/* Avatar */}
          <div className={`-mt-10 w-20 h-20 rounded-2xl ${meta.bg} ${meta.text} text-3xl flex items-center justify-center border-4 border-white shadow-sm mb-4`}>
            {meta.icon}
          </div>
          ...
          <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
            <div>
              ...
              <p className="text-indigo-600 font-semibold mt-0.5">{artisan.specialty}</p>
            </div>
            <span className={`badge ${artisan.available ? 'badge-available' : 'badge-busy'} text-sm`}>
              <span className={`w-2 h-2 rounded-full ${artisan.available ? 'bg-emerald-500' : 'bg-slate-400'}`} />
          ...
          <div className="flex flex-wrap gap-4 mb-6 text-sm text-slate-600">
            {artisan.city && (
              <span className="flex items-center gap-1.5">
                <MapPin size={15} className="text-slate-400" /> {artisan.city}
              </span>
            )}
            {artisan.phone && (
              <span className="flex items-center gap-1.5">
                <Phone size={15} className="text-slate-400" /> {artisan.phone}
              </span>
            )}
            {artisan.hourlyRate && (
              <span className="flex items-center gap-1.5 font-semibold text-slate-800">
                <Euro size={15} className="text-slate-400" /> {artisan.hourlyRate} / hour
              </span>
            )}
          </div>
          ...
          {user?.role === 'client' ? (
            <div className="flex gap-3">
              <Link to={`/demande/${artisan.userId}`} className="btn btn-primary flex-1 justify-center">
